...
void CMAObject::loadSfoMetadata(const QString &path)
{
    ...
    if(!skipMetadata && reader.load(sfo)) {
        ...
        metadata.data.saveData.dateTimeUpdated = QFileInfo(sfo).metadataChangeTime().toUTC().toTime_t();
    } else {
...
void CMAObject::initObject(const QFileInfo &file, int obj_file_type)
{
    ...
    metadata.dateTimeCreated = file.birthTime().toUTC().toTime_t();
    metadata.size = 0;
    ...
    if(MASK_SET(metadata.dataType, SaveData | Folder)) {
        ...
        metadata.data.video.dateTimeUpdated = file.metadataChangeTime().toUTC().toTime_t();
        ...
        metadata.data.photo.dateTimeOriginal = file.birthTime().toUTC().toTime_t();
